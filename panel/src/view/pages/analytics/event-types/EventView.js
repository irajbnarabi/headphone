...
function EventView() {
	...
	const [form, setFormRaw] = useState(eventConfigs.eventTypes.VIEW.body);
	...
	const setForm = (keyVal) => {
		setFormRaw({ ...eventConfigs.eventTypes.VIEW.body, ...form, ...keyVal });
	};

	useEffect(() => {
		const from = moment(fromDate).unix();
		const to = moment(toDate).unix();
		dispatch(analyticsActions.getData(eventConfigs.eventTypes.VIEW.eventName, from, to, eventConfigs.eventTypes.VIEW.body));
		dispatch(analyticsActions.getHotData(eventConfigs.eventTypes.VIEW.eventName, from, to, eventConfigs.eventTypes.VIEW.body));
	}, []);
	...
	useEffect(() => {
		dispatch(programActions.searchProgram(form.type, searchProgramQuery));
	}, [searchProgramQuery, form.type]);

	useEffect(() => {
		dispatch(tagActions.searchTag(searchTagQuery));
	}, [searchTagQuery]);
	...
	const renderChart = () => {
		...
		return <LineChart data={dataset} chartLabel={eventConfigs.eventTypes.VIEW.eventNameFa} labels={labels} />;
	};
	...
	const renderTable = () => {
		...
		const handlePagination = (selectedPage) => {
			...
			const from = moment(fromDate).unix();
			const to = moment(toDate).unix();
			dispatch(analyticsActions.getHotData(eventConfigs.eventTypes.VIEW.eventName, from, to, form, selectedPage));
		};
	...
	const renderForm = () => {
		const handleSubmit = () => {
			const from = moment(fromDate).unix();
			const to = moment(toDate).unix();
			dispatch(analyticsActions.getData(eventConfigs.eventTypes.VIEW.eventName, from, to, form));
			dispatch(analyticsActions.getHotData(eventConfigs.eventTypes.VIEW.eventName, from, to, form));
		};
		...
		return (
			<form>
				...
				<Autocomplete
					...
					fullWidth
					onInputChange={(e, value) => setSearchTagQuery(value)}
					onChange={(e, value) => setForm({ genre: value?.value ? value.value : '' })}
					getOptionLabel={(option) => option.value}
					freeSolo
					selectOnFocus
					clearOnBlur
					handleHomeEndKeys
					renderInput={(params) => <TextField {...params} label='ژانر' variant='outlined' />}
				/>
	...
	return (
		<div className='analytics-base-container'>
			<b>{eventConfigs.eventTypes.VIEW.eventNameFa}</b>
