...
function EventLogin() {
	...
	const [form] = useState(eventConfigs.eventTypes.LOGIN.body);
	...
	useEffect(() => {
		...
		dispatch(analyticsActions.getData(eventConfigs.eventTypes.LOGIN.eventName, from, to, eventConfigs.eventTypes.LOGIN.body));
	}, []);
	...
	const renderChart = () => {
		...
		return <LineChart data={dataset} chartLabel={eventConfigs.eventTypes.LOGIN.eventNameFa} labels={labels} />;
	};
	...
	const renderForm = () => {
		const handleSubmit = () => {
			const from = moment(fromDate).unix();
			const to = moment(toDate).unix();
			dispatch(analyticsActions.getData(eventConfigs.eventTypes.LOGIN.eventName, from, to, form));
		};
	...
	return (
		<div className='analytics-base-container'>
			<b>{eventConfigs.eventTypes.LOGIN.eventNameFa}</b>
